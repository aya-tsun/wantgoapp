...
export default function ItemCard({ item, onEdit }) {
  ...
  return (
    <div
      onClick={() => onEdit(item)}
      className="bg-white border border-zinc-200 p-4 cursor-pointer hover:border-zinc-400 hover:shadow-sm transition-all group"
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <h3 className="font-medium text-zinc-900 text-sm leading-snug flex-1 line-clamp-2">
          {item.title}
        </h3>
      ...
      <div className="text-xs text-zinc-400 mb-3">
        ...
        {item.ticket_status && item.ticket_status !== '未対応' && (
          <span className="ml-2 text-amber-500">· 予約: {item.ticket_status}</span>
        )}
      </div>
      ...
      <div className="space-y-1.5 text-xs text-zinc-500">
        ...
        {item.url && (
          <div className="flex items-center gap-1.5 overflow-hidden">
            ...
            <a
              ...
              className="text-zinc-500 hover:text-zinc-900 underline underline-offset-2 truncate transition-colors"
            >
        ...
        {item.memo && (
          <div className="flex items-start gap-1.5">
            <IconNote />
            <span className="line-clamp-2 text-zinc-400">{item.memo}</span>
          </div>
        )}
      </div>
    </div>
